...
export const EditProductScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        caseDiameter: product.caseDiameter?.toString() || '',
        strapMaterial: product.strapMaterial || '',
        waterResistance: product.waterResistance?.toString() || '',
        color: product.color || '',
  ...
  const handleSubmit = async () => {
    ...
    if (!product || !product.id) {
      setErrorMessage('Product ID is missing. Cannot update.');
      return;
    }
    ...
    try {
      ...
      const response = await axios.put(
        `${API_CONFIG.BASE_URL}/watches/${product.id}`,
        productData,
        {
          headers: {
      ...
      setErrorMessage(error?.response?.data?.error?.message || 'Failed to update product');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={tw`flex-1 bg-gray-50`}>
      <View style={tw`bg-white px-6 py-4 border-b border-gray-200`}>
        <Text style={tw`text-2xl font-bold`}>Edit Product</Text>
      </View>
      ...
      <ScrollView style={tw`flex-1 px-6 py-4`}>
        ...
        <TouchableOpacity
          ...
          style={tw`bg-blue-600 py-4 rounded-xl mb-6`}
          disabled={loading}
        >
